...
struct WritesSkewedRangedReadsWorkload : TestWorkload {
	...
	ContinuousSample<double> latencies, readLatencies, commitLatencies, commitRangedLatencies, GRVLatencies,
	    fullReadLatencies;
	...
	WritesSkewedRangedReadsWorkload(WorkloadContext const& wcx)
	  : TestWorkload(wcx), latencies(sampleSize), readLatencies(sampleSize), fullReadLatencies(sampleSize),
	    commitLatencies(sampleSize), commitRangedLatencies(sampleSize), GRVLatencies(sampleSize), readLatencyTotal(0),
	    ...
	    rangedReadTransactions("Read Range Transactions"), rangedReadRetries("Read Range Retries"),
	    clearRewriteRangeTransactions("Commit Range Transactions"), clearRewriteRangeRetries("Commit Range Retries"),
...
#ifndef GETOPTION
#define GETOPTION(PNAME, DVAL) PNAME = getOption(options, LiteralStringRef(#PNAME), DVAL)
#define GETOPTION_D(VNAME, PNAME, DVAL) VNAME = getOption(options, LiteralStringRef(#PNAME), DVAL)
#endif
		...
		vector<std::string> insertionCountsToMeasureString =
		    getOption(options, LiteralStringRef("insertionCountsToMeasure"), vector<std::string>());
	...
	void getMetrics(vector<PerfMetric>& m) override {
		...
		m.push_back(
		    PerfMetric("Transactions/sec", (aTransactions.getValue() + bTransactions.getValue()) / duration, false));
		...
		if (!rampUpLoad) {
			...
			m.push_back(
			    PerfMetric("Median Total Read Latency (ms, averaged)", 1000 * fullReadLatencies.median(), true));
			...
			m.push_back(PerfMetric("Mean Commit Range Latency (ms)", 1000 * commitRangedLatencies.mean(), true));
			m.push_back(
			    PerfMetric("Median Commit Range Latency (ms, averaged)", 1000 * commitRangedLatencies.median(), true));

			m.push_back(PerfMetric("Mean Read Range Latency (ms)", 1000 * readRangeLatencies.mean(), true));
			m.push_back(
			    PerfMetric("Median Read Range Latency (ms, averaged)", 1000 * readRangeLatencies.median(), true));
			m.push_back(
			    PerfMetric("5% Read Range Latency (ms, averaged)", 1000 * readRangeLatencies.percentile(.05), true));
			m.push_back(
			    PerfMetric("95% Read Range Latency (ms, averaged)", 1000 * readRangeLatencies.percentile(.95), true));
		}
		...
		m.push_back(PerfMetric("Bytes read/sec",
		                       (reads * (keyBytes + (minValueBytes + maxValueBytes) * 0.5)) / duration, false));
		m.push_back(PerfMetric("Bytes written/sec",
		                       (writes * (keyBytes + (minValueBytes + maxValueBytes) * 0.5)) / duration, false));
		m.insert(m.end(), periodicMetrics.begin(), periodicMetrics.end());

		vector<std::pair<uint64_t, double>>::iterator ratesItr = ratesAtKeyCounts.begin();
	...
	KeyRange getRandomKeyRange(uint64_t sizeLimit) {
		...
		uint64_t endLocation =
		    ...
		    deterministicRandom()->randomInt(
		        0, 1 + std::min(sizeLimit, std::min(nodeCount - startLocation, (uint64_t)(1) << scale)));

		return KeyRangeRef(keyForIndex(startLocation), keyForIndex(endLocation));
	}

	Standalone<KeyValueRef> operator()(uint64_t n) { return KeyValueRef(keyForIndex(n), randomValue()); }

	ACTOR static Future<Void> tracePeriodically(WritesSkewedRangedReadsWorkload* self) {
		...
		loop {
			...
			if (self->readLatencyCount)
				TraceEvent("RwRowReadLatency")
				    .detail("MilliSeconds", self->readLatencyTotal / self->readLatencyCount * 1000.)
				    .detail("Count", self->readLatencyCount)
				    .detail("Elapsed", elapsed);

			TraceEvent("RwGRVLatency").detail("MilliSeconds", 1000 * self->GRVLatencies.mean());
			TraceEvent("RwCommitLatency").detail("MilliSeconds", 1000 * self->commitLatencies.mean());
			TraceEvent("RwCommitRangedLatency").detail("MilliSeconds", 1000 * self->commitRangedLatencies.mean());
			...
			if (recordBegin && recordEnd) {
				...
				self->periodicMetrics.push_back(
				    PerfMetric(ts + "Operations/sec", (ops - last_ops) / self->periodicLoggingInterval, false));

				if (self->rampUpLoad) {
					self->periodicMetrics.push_back(
					    PerfMetric(ts + "Mean Latency (ms)", 1000 * self->latencies.mean(), true));
					self->periodicMetrics.push_back(
					    PerfMetric(ts + "Median Latency (ms, averaged)", 1000 * self->latencies.median(), true));
					self->periodicMetrics.push_back(
					    PerfMetric(ts + "5% Latency (ms, averaged)", 1000 * self->latencies.percentile(.02), true));
					self->periodicMetrics.push_back(
					    PerfMetric(ts + "95% Latency (ms, averaged)", 1000 * self->latencies.percentile(.95), true));

					self->periodicMetrics.push_back(
					    PerfMetric(ts + "Mean Row Read Latency (ms)", 1000 * self->readLatencies.mean(), true));
					self->periodicMetrics.push_back(PerfMetric(ts + "Median Row Read Latency (ms, averaged)",
					                                           1000 * self->readLatencies.median(), true));
					self->periodicMetrics.push_back(PerfMetric(ts + "5% Row Read Latency (ms, averaged)",
					                                           1000 * self->readLatencies.percentile(.05), true));
					self->periodicMetrics.push_back(PerfMetric(ts + "95% Row Read Latency (ms, averaged)",
					                                           1000 * self->readLatencies.percentile(.95), true));

					self->periodicMetrics.push_back(
					    PerfMetric(ts + "Mean Total Read Latency (ms)", 1000 * self->fullReadLatencies.mean(), true));
					self->periodicMetrics.push_back(PerfMetric(ts + "Median Total Read Latency (ms, averaged)",
					                                           1000 * self->fullReadLatencies.median(), true));
					self->periodicMetrics.push_back(PerfMetric(ts + "5% Total Read Latency (ms, averaged)",
					                                           1000 * self->fullReadLatencies.percentile(.05), true));
					self->periodicMetrics.push_back(PerfMetric(ts + "95% Total Read Latency (ms, averaged)",
					                                           1000 * self->fullReadLatencies.percentile(.95), true));

					self->periodicMetrics.push_back(
					    PerfMetric(ts + "Mean GRV Latency (ms)", 1000 * self->GRVLatencies.mean(), true));
					self->periodicMetrics.push_back(
					    PerfMetric(ts + "Median GRV Latency (ms, averaged)", 1000 * self->GRVLatencies.median(), true));
					self->periodicMetrics.push_back(PerfMetric(ts + "5% GRV Latency (ms, averaged)",
					                                           1000 * self->GRVLatencies.percentile(.05), true));
					self->periodicMetrics.push_back(PerfMetric(ts + "95% GRV Latency (ms, averaged)",
					                                           1000 * self->GRVLatencies.percentile(.95), true));

					self->periodicMetrics.push_back(
					    PerfMetric(ts + "Mean Commit Latency (ms)", 1000 * self->commitLatencies.mean(), true));
					self->periodicMetrics.push_back(PerfMetric(ts + "Median Commit Latency (ms, averaged)",
					                                           1000 * self->commitLatencies.median(), true));
					self->periodicMetrics.push_back(PerfMetric(ts + "Mean Commit Range Latency (ms)",
					                                           1000 * self->commitRangedLatencies.mean(), true));
					self->periodicMetrics.push_back(PerfMetric(ts + "Median Commit Range Latency (ms, averaged)",
					                                           1000 * self->commitRangedLatencies.median(), true));

					self->periodicMetrics.push_back(PerfMetric(ts + "5% Commit Latency (ms, averaged)",
					                                           1000 * self->commitLatencies.percentile(.05), true));
					self->periodicMetrics.push_back(PerfMetric(ts + "95% Commit Latency (ms, averaged)",
					                                           1000 * self->commitLatencies.percentile(.95), true));

					self->periodicMetrics.push_back(PerfMetric(ts + "5% Commit Ranged Latency (ms, averaged)",
					                                           1000 * self->commitRangedLatencies.percentile(.05),
					                                           true));
					self->periodicMetrics.push_back(PerfMetric(ts + "95% Commit Ranged Latency (ms, averaged)",
	...
	ACTOR static Future<Void> logLatency(Future<Optional<Value>> f, ContinuousSample<double>* latencies,
	                                     double* totalLatency, int* latencyCount, bool shouldRecord) {
	...
	Future<Void> readOp(Trans* tr, std::vector<int> keys, WritesSkewedRangedReadsWorkload* self, bool shouldRecord) {
		if (!keys.size()) return Void();
		if (!self->dependentReads) {
			if (self->rangeReads) {
				Standalone<VectorRef<KeyRangeRef>> keyRanges;
				for (int op = 0; op < keys.size(); op++) {
					keyRanges.push_back_deep(keyRanges.arena(), KeyRangeRef(self->keyForIndex(keys[op]),
				...
				for (int op = 0; op < keys.size(); op++)
					readers.push_back(logLatency(tr->get(self->keyForIndex(keys[op])), &self->readLatencies,
					                             &self->readLatencyTotal, &self->readLatencyCount, shouldRecord));
	...
	// sort
	ACTOR template <class Trans>
	Future<Void> readRangeOp(Trans* tr, Standalone<VectorRef<KeyRangeRef>> keyRanges,
	                         WritesSkewedRangedReadsWorkload* self, bool shouldRecord) {
		std::vector<Future<Void>> readers;
		state Standalone<VectorRef<KeyRangeRef>> theKeyRanges = keyRanges;

		for (auto kr : theKeyRanges) {
			readers.push_back(logLatency(tr->getRange(kr, GetRangeLimits(-1, 1000000)), &self->readLatencies,
			                             &self->readLatencyTotal, &self->readLatencyCount, shouldRecord));
		}
		wait(waitForAll(readers));
	...
	ACTOR Future<Void> _setup(Database cx, WritesSkewedRangedReadsWorkload* self) {
	...
	ACTOR Future<Void> _start(Database cx, WritesSkewedRangedReadsWorkload* self) {
		...
		// ranged read actors
		double rangedReadPercent = std::min(self->rangedReadActorPercent / 100, (double)1);
		double rangedReadSkewPercent = std::min(self->rangedReadPercentSkewed / 100, (double)1);
		int rangedReadActorTotalCount = rangedReadPercent * self->actorCount;
		int rangedReadActorSkewedCount = rangedReadActorTotalCount * rangedReadSkewPercent;

		double rangedWritePercent = std::min(self->rangedWriteActorPercent / 100, (double)1);
		double rangedWriteSkewPercent = std::min(self->rangedWritePercentSkewed / 100, (double)1);
		int rangedWriteActorTotalCount = rangedWritePercent * self->actorCount;
		int rangedWriteActorSkewedCount = rangedWriteActorTotalCount * rangedWriteSkewPercent;

		int rwActorCount = std::max(self->actorCount - rangedReadActorTotalCount - rangedWriteActorTotalCount, 0);
		int rangedReadActorCount = rangedReadActorTotalCount - rangedReadActorSkewedCount;
		int rangedWriteActorCount = rangedWriteActorTotalCount - rangedWriteActorSkewedCount;
		...
		printf("SkewedRandomRangedReadWrite percent:%f rangedWrite percent:%f "
		       ...
		       rangedReadPercent, rangedWritePercent, rangedReadActorCount, rangedReadActorSkewedCount,
		       rangedWriteActorCount, rangedWriteActorSkewedCount, rwActorCount);
		...
		for (int c = 0; c < rwActorCount; c++) {
			...
			if (self->useRYW)
				...
				worker =
				    self->randomReadWriteClient<Transaction>(cx, self, self->actorCount / self->transactionsPerSecond);
			clients.push_back(worker);
		}
		for (int c = 0; c < rangedReadActorCount; c++) {
			Future<Void> worker;
			worker =
			    self->rangedReadClient<Transaction>(cx, self, self->actorCount / self->transactionsPerSecond, false, 0);
			clients.push_back(worker);
		}
		for (int c = 0; c < rangedWriteActorCount; c++) {
			Future<Void> worker;
			worker = self->rangedClearAndWriteClient<Transaction>(
			    cx, self, Standalone<VectorRef<int>>(), self->actorCount / self->transactionsPerSecond, false);
			clients.push_back(worker);
		}
		...
		for (int k = 0; k < numHotSpots; k++) {
			...
			skewedKeys.push_back(
			    skewedKeys.arena(),
			    self->getRandomKey(self->nodeCount - (self->rangedReadMaxKeyRange + self->rangedReadSkewStartRange),
			                       self->useCommonSkewKeys));
			TraceEvent("SkewedRandomRangedReadWriteAddingSkewedKey").detail("StartIndex", skewedKeys[k]);
		...
		for (int c = 0; c < rangedReadActorSkewedCount; c++) {
			...
			worker = self->rangedReadClient<Transaction>(cx, self, self->actorCount / self->transactionsPerSecond, true,
			                                             skewedKeys[skewedKeyIndex]);
			clients.push_back(worker);
		}
		for (int c = 0; c < rangedWriteActorSkewedCount; c++) {
			Future<Void> worker;
			worker = self->rangedClearAndWriteClient<Transaction>(cx, self, skewedKeys,
	...
	int getRandomKey(int nodeCount, bool commonAcrossTesters = false) {
		Reference<IRandom> my_g_random = commonAcrossTesters ? common_tester_g_random : deterministicRandom();

		if (forceHotProbability && my_g_random->random01() < forceHotProbability)
	...
	ACTOR template <class Trans>
	Future<Void> rangedReadClient(Database cx, WritesSkewedRangedReadsWorkload* self, double delay, bool skewed,
	                              int skewedKey) {
		state double startTime = now();
		...
		loop {
			...
			if (self->rangedReadDoSkewMigration && isSkewed) {
				double skewIncreaseBy = (lastTime - lastSkewMigrateTime) * self->rangedReadSkewMigrationRate;
				...
				if (skewIncreaseBy > 1) {
					...
					actingSkewedKey =
					    ...
					    (self->nodeCount - (self->rangedReadMaxKeyRange + self->rangedReadSkewStartRange));
					lastSkewMigrateTime = now();
				}
			}

			if (!self->rampUpLoad ||
			    (currentSweep % 2 == 0 && deterministicRandom()->random01() <= numSweeps - currentSweep) ||
			    (currentSweep % 2 == 1 && deterministicRandom()->random01() <= 1 - (numSweeps - currentSweep))) {
				...
				loop {
					try {
						...
						if (self->shouldRecord()) self->GRVLatencies.addSample(now() - GRVStartTime);
						state double readStart = now();
						Standalone<VectorRef<KeyRangeRef>> keyRanges;
						int startKey, endKey;
						for (auto op = 0; op < reads; op++) {
							if (isSkewed) {
								int startAdd = deterministicRandom()->randomInt(0, self->rangedReadSkewStartRange);
								int keyRange = deterministicRandom()->randomInt(self->rangedReadMinKeyRange,
							...
							keyRanges.push_back_deep(
							    keyRanges.arena(), KeyRangeRef(self->keyForIndex(startKey), self->keyForIndex(endKey)));
						}
						wait(self->readRangeOp(&tr, keyRanges, self, self->shouldRecord()));
						if (self->shouldRecord()) self->readRangeLatencies.addSample(now() - readStart);
						break;
					} catch (Error& e) {
						wait(tr.onError(e));
	...
	// KeyRange parameters
	ACTOR template <class Trans>
	Future<Void> rangedClearAndWriteClient(Database cx, WritesSkewedRangedReadsWorkload* self,
		...
		actingSkewedKeys.append(actingSkewedKeys.arena(), skewedReadKeys.begin(),
		...
		loop {
			...
			if (self->rangedReadDoSkewMigration && isSkewed) {
				...
				if (skewIncreaseBy > 1) {
					...
					while (actingSkewedKeys.size()) actingSkewedKeys.pop_back();
			...
			if (!self->rampUpLoad ||
				...
				loop {
					try {
						...
						state int endKey;
						state int numKeys;
						numKeys = deterministicRandom()->randomInt(self->rangedWriteMinKeyRange,
						                                           self->rangedWriteMaxKeyRange);
						if (isSkewed) {
							int skewedReadKeyId = 0;
							if (self->rangedReadSkewNumHotSpots > 1) {
								skewedReadKeyId =
								    deterministicRandom()->randomInt(0, self->rangedReadSkewNumHotSpots - 1);
							}
							int skewedReadKey = actingSkewedKeys[skewedReadKeyId];
							startKey = deterministicRandom()->randomInt(
							    0, std::max(0, skewedReadKey - self->rangedWriteMinKeyRange - 1));
						} else {
							startKey = self->getRandomKey(self->nodeCount);
						}
						endKey = startKey + numKeys;
						state KeyRange keyRange = KeyRangeRef(self->keyForIndex(startKey), // must be standalone???
						...
						if (!self->rangedWriteBlind) {
							wait(logLatency(tr.getRange(keyRange, GetRangeLimits(-1, 1000000)), &self->readLatencies,
							                &self->readLatencyTotal, &self->readLatencyCount, self->shouldRecord()));
						}
						tr.clear(keyRange);
						...
						if (self->shouldRecord()) self->commitRangedLatencies.addSample(now() - commitStart);
						break;
					} catch (Error& e) {
						wait(tr.onError(e));
						if (self->shouldRecord()) {
	...
	ACTOR template <class Trans>
	Future<Void> randomReadWriteClient(Database cx, WritesSkewedRangedReadsWorkload* self, double delay) {
		state double startTime = now();
		state double lastTime = now();
		state double GRVStartTime;
		state UID debugID;
			...
			if (!self->rampUpLoad ||
				...
				loop {
					try {
						...
						if (self->shouldRecord()) self->fullReadLatencies.addSample(now() - readStart);

						if (!writes) break;

						if (self->adjacentWrites) {
							int startKey = self->getRandomKey(self->nodeCount - writes);
							for (int op = 0; op < writes; op++) tr.set(self->keyForIndex(startKey + op), values[op]);
						} else {
							for (int op = 0; op < writes; op++)
								tr.set(self->keyForIndex(self->getRandomKey(self->nodeCount)), values[op]);
						}
						...
						if (self->shouldRecord()) self->commitLatencies.addSample(now() - commitStart);
						break;
					} catch (Error& e) {
						wait(tr.onError(e));
...
WorkloadFactory<WritesSkewedRangedReadsWorkload> WritesSkewedRangedReadsWorkloadFactory("WritesSkewedRangedReads");
